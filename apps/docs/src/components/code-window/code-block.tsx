...
import highlightLine from '@site/src/libs/rehype-highlight-line';
import highlightWord from '@site/src/libs/rehype-highlight-word';
...
function getTextNodes(node: any): any[] {
  const childTextNodes: React.ReactNode[] = [];
...
function CodeTypewriter({ value, className, css, ...props }: any) {
  ...
  React.useEffect(() => {
    ...
    if (wrapper) {
      const allTextNodes = getTextNodes(wrapper);
