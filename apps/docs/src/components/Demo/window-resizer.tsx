...
import { useIsomorphicLayoutEffect } from '@site/src/hooks/useIsomorphicLayoutEffect';
import { WindowActions } from '@site/src/components/code-window/window-actions';
...
const WindowResizer: React.FC<WindowResizerProps> = props => {
  const constraintsResizerRef = React.useRef<HTMLDivElement>(null);
  ...
  useIsomorphicLayoutEffect(() => {
    const observer = new window.ResizeObserver(() => {
      if (constraintsResizerRef.current && resizerRef.current) {
        const width = constraintsResizerRef.current.offsetWidth - resizerRef.current.offsetWidth;
    ...
    <div className={main()} style={{ height }}>
      ...
      {resizeEnabled && (
        <div
          ...
          className={base({
            className: 'z-1 xs:w-mw-xs bottom-0 right-0 top-0',
          })}
          style={{
